...
The `trashwheel_data` data has `r nrow(trashwheel_data)` observations and `r ncol(trashwheel_data)` variables and describes the trash collected by different water wheels in the harbor of Baltimore, MD. The average weight (in tons) collected by the three trashwheels, Mr. Trashwheel, Professor Trashwheel, and Gwynnda, is `r mean(pull(trashwheel_data, weight_tons))`. The average number of homes powered by all three trash wheels is `r mean(pull(trashwheel_data, homes_powered))`



## Total weight of trash collected by Prof Trashwheel is `r sum(pull(prof_trashwheel_data, weight_tons))`.


 
## Total number of cigarette butts collected by Gwynnda in 2021 is `r sum(pull(filter(gwynnda_data, month == "July", year == "2021"), cigarette_butts))`
...
MCI_baseline_final <- subset(MCI_baseline, age_at_onset >= current_age | age_at_onset == ".")

names(MCI_baseline_final)[names(MCI_baseline_final) == "id"] <- "study_id"


```

The `MCI_baseline_final` data has `r nrow(MCI_baseline_final)` observations and `r ncol(MCI_baseline_final)` variables....... This means that `r nrow(MCI_baseline_final)` participants were recruited.

Of these participants, `r nrow(subset(MCI_baseline_final, age_at_onset != "."))` developed MCI.

The average age at the study baseline is `r mean(pull(MCI_baseline_final, current_age))`

`r sum(with(MCI_baseline_final, apoe4 == 1)) / nrow(MCI_baseline_final) * 100`% of women in the study are APOE4 carriers.
...
```
DATA DESCRIPTION
Yes, some participants only appear in the cleaned amyloid dataset and not hte cleaned baseline dataset, as the amyloid dataset has 488 observations and the baseline only has 480. 
